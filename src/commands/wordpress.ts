...
export async function postToBlog(chatId: number, command: any) {
  ...
  const prompt = `Write a blog post (800-1200 words) promoting the Website DNA™ ebook. Use this vault for context:\n${vault}\n\nAngle: ${hook.title}\nDetails: ${hook.lines.join("\n")}\n\nStart with a headline as the first line starting with "# ". Include the link as SELAR_LINK_PLACEHOLDER. Follow the tone rules. Use plain paragraphs, not markdown (WordPress accepts HTML, but we'll keep it simple).`;
